...
└───────────────────────────────┬───────────────────────────────┬──────────────┘
                                ...
                                ▼                               ▼
                ┌──────────────────────────┐      ┌──────────────────────────────┐
                │   Relational DB          │      │      Python Worker Pool      │
                │     (PostgreSQL)         │      │        (FastAPI)             │
...
### Table: `sessions`
The `sessions` table stores refresh token sessions (hashed) to enable secure logout and token rotation.
| Field | Type | Description |
|---|---|---|
| id | **INT PRIMARY KEY** | Unique session identifier. |
| user_id | *INT FOREIGN KEY* | References `users.id` (session owner). |
| refresh_token_hash | VARCHAR(64) | SHA-256 hash of the refresh token (never stored in plaintext). |
| created_at | TIMESTAMP | Session creation time. |
| expires_at | TIMESTAMP | When the refresh token expires (e.g., 7 days). |
| revoked_at | TIMESTAMP | When the session was revoked (NULL if active). |
| user_agent | TEXT | Client user-agent for device/audit info. |
| ip | VARCHAR(45) | Client IP address for audit info. |
...
### Relationships
- `users (1) -> (N) sessions` via `sessions.user_id`
---
